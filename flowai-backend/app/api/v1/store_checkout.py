...
def checkout(payload:CheckoutRequest,db:Session=Depends(get_db),user:User=Depends(require_customer_dep)):
    ...
    order=order_service.create(db,user.id,{k:getattr(address,k) for k in ("full_name","line1","line2","city","state","postal_code","country")},payload.idempotency_key)
